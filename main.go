...
func main() {
	tocFile := flag.String("toc", "TOC.md", "Markdown file to start parsing")
	replacePrefix := flag.String("rpr", "", "Url prefix to replace as current dir")
	...
	if "" != *replacePrefix {
		replaceMap[*replacePrefix] = rdir
	}

	files := []string{filepath.Join(".", *tocFile)}
